...
int main(void) {
    ...
    assert(model_only);
    assert(mcp_tool_set_visibility(ctx, app_only, MCP_TOOL_VIS_APP) == MCP_OK);
    assert(mcp_tool_set_visibility(ctx, model_only, MCP_TOOL_VIS_MODEL) == MCP_OK);
    assert(mcp_server_add_tool(ctx, srv, echo) == MCP_OK);
    assert(mcp_server_add_tool(ctx, srv, app_only) == MCP_OK);
    assert(mcp_server_add_tool(ctx, srv, model_only) == MCP_OK);
    ...
    assert(mcp_session_set_apps_host(ctx, apps, true) == MCP_OK);
    assert(!mcp_session_is_apps_host(ctx, plain));
    assert(mcp_session_is_apps_host(ctx, apps));

    // default visibility: echo visible to both
    assert(list_size(ctx, srv, plain, "l1") == 2);
    assert(list_size(ctx, srv, apps, "l2") == 2);
    assert(list_has(ctx, srv, plain, "l3", "echo"));
    assert(list_has(ctx, srv, plain, "l4", "model_only"));
    assert(!list_has(ctx, srv, plain, "l5", "app_only"));
    assert(list_has(ctx, srv, apps, "l6", "app_only"));
    assert(!list_has(ctx, srv, apps, "l7", "model_only"));

    // hidden tool call -> -32601 (no existence leak)
    mcp_message_t *r = dispatch_new(ctx, srv, plain, "c1", "tools/call",
                                    call_params(ctx, "app_only"));
    assert(error_code(ctx, r) == (int)MCP_RPC_METHOD_NOT_FOUND);
    mcp_message_destroy(ctx, r);
    ...
    // NULL / range guards
    assert(mcp_tool_set_visibility(ctx, NULL, MCP_TOOL_VIS_APP) == MCP_ERR_INVALID_ARGUMENT);
    assert(mcp_tool_set_visibility(NULL, echo, MCP_TOOL_VIS_APP) == MCP_ERR_INVALID_ARGUMENT);
    assert(mcp_tool_set_visibility(ctx, echo, (mcp_tool_visibility_t)99) ==
           MCP_ERR_INVALID_ARGUMENT);
    assert(mcp_tool_require_perms(ctx, NULL, 1) == MCP_ERR_INVALID_ARGUMENT);
    assert(mcp_session_set_apps_host(ctx, NULL, true) == MCP_ERR_INVALID_ARGUMENT);
    assert(!mcp_session_is_apps_host(ctx, NULL));
    assert(mcp_session_grant(ctx, NULL, 1) == MCP_ERR_INVALID_ARGUMENT);
    assert(mcp_session_revoke(ctx, NULL, 1) == MCP_ERR_INVALID_ARGUMENT);
    assert(!mcp_session_grants(ctx, NULL, 1));
