...
int main(void) {
    ...
    mcp_json_value_t *name = mcp_json_parse(NULL, "\"caf\\u00e9 \\ud83d\\ude00\"",
                                            strlen("\"caf\\u00e9 \\ud83d\\ude00\""));
    CHECK(name != NULL);
    CHECK(mcp_json_object_set(NULL, doc, "name", name) == MCP_OK);
    mcp_json_value_t *num = mcp_json_number_new(NULL, -1.5e10);
    CHECK(num != NULL);
    CHECK(mcp_json_object_set(NULL, doc, "n", num) == MCP_OK);
    CHECK(mcp_json_object_set(NULL, doc, "t", mcp_json_bool_new(NULL, true)) == MCP_OK);
    CHECK(mcp_json_object_set(NULL, doc, "z", mcp_json_null_new(NULL)) == MCP_OK);
    mcp_json_value_t *arr = mcp_json_array_new(NULL);
    CHECK(arr != NULL);
    CHECK(mcp_json_array_append(NULL, arr, mcp_json_number_new(NULL, 1.0)) == MCP_OK);
    mcp_json_value_t *ab = mcp_json_parse(NULL, "\"a\\\\b\"", 6);
    CHECK(ab != NULL);
    CHECK(mcp_json_array_append(NULL, arr, ab) == MCP_OK);
    CHECK(mcp_json_array_append(NULL, arr, mcp_json_object_new(NULL)) == MCP_OK);
    CHECK(mcp_json_object_set(NULL, doc, "arr", arr) == MCP_OK);

    char *s1 = mcp_json_serialize(NULL, doc);
    CHECK(s1 != NULL);
    CHECK(strstr(s1, "\xf0\x9f\x98\x80") != NULL);
