...
static void expect_code(mcp_context_t *ctx, const char *text, int want) {
    ...
    assert(code == want);
    assert((want == 0 && st == MCP_OK) || (want != 0 && st != MCP_OK));
    // NULL out-param is allowed
    assert(mcp_message_validate(ctx, m, NULL) == st);
...
int main(void) {
    ...
    assert(parse(ctx, "{\"jsonrpc\":") == NULL);
    // 3. root array → parse NULL (not an object envelope)
    assert(parse(ctx, "[1,2]") == NULL);
    ...
    assert(mcp_method_known("tools/call"));
    assert(mcp_method_known("notifications/initialized"));
    assert(!mcp_method_known("frobnicate"));
    assert(!mcp_method_known(NULL));
    ...
    memset(big, ' ', big_n);
    big[big_n] = '\0';
    assert(mcp_message_parse(ctx, big, big_n) == NULL);
    free(big);

    // 6. invalid UTF-8 → parse NULL
    assert(parse(ctx, "{\"a\":\"\xff\"}") == NULL);

    // 7. duplicate id via idset
    mcp_idset_t *ids = mcp_idset_create(ctx);
    assert(ids != NULL);
    assert(mcp_idset_add(ctx, ids, MCP_ID_STRING, "r1", 0) == MCP_OK);
    assert(mcp_idset_contains(ctx, ids, MCP_ID_STRING, "r1", 0));
    assert(mcp_idset_add(ctx, ids, MCP_ID_STRING, "r1", 0) == MCP_ERR_ALREADY_EXISTS);
    assert(mcp_idset_add(ctx, ids, MCP_ID_NUMBER, NULL, 1) == MCP_OK);
    assert(mcp_idset_add(ctx, ids, MCP_ID_NUMBER, NULL, 1) == MCP_ERR_ALREADY_EXISTS);
    // string "1" and number 1 are distinct ids
    assert(mcp_idset_add(ctx, ids, MCP_ID_STRING, "1", 0) == MCP_OK);
    assert(!mcp_idset_contains(ctx, ids, MCP_ID_NUMBER, NULL, 2));
    mcp_idset_remove(ctx, ids, MCP_ID_STRING, "r1", 0);
    assert(!mcp_idset_contains(ctx, ids, MCP_ID_STRING, "r1", 0));
    assert(mcp_idset_add(ctx, ids, MCP_ID_STRING, "r1", 0) == MCP_OK);
    assert(mcp_idset_add(ctx, ids, MCP_ID_NONE, NULL, 0) == MCP_ERR_INVALID_ARGUMENT);
    ...
    assert(resp != NULL);
    int rc = -1;
    assert(mcp_validate_method(ctx, resp, &rc) == MCP_OK && rc == 0);
    assert(mcp_validate_params(ctx, resp, &rc) == MCP_OK && rc == 0);
