...
static void server_step(pump_t *p) {
    ...
    assert(fseek(p->c2s, p->cpos, SEEK_SET) == 0);
    static char buf[16384];
    assert(fgets(buf, (int)sizeof(buf), p->c2s) != NULL);
    p->cpos = ftell(p->c2s);
    mcp_message_t *msg = mcp_message_parse(ctx, buf, strlen(buf));
    assert(msg != NULL);
    if (mcp_message_kind(ctx, msg) == MCP_MSG_NOTIFICATION) {
        assert(mcp_server_notify(ctx, p->srv, p->sess, msg) == MCP_OK);
        mcp_message_destroy(ctx, msg);
        return;
    }
    mcp_message_t *resp = NULL;
    assert(mcp_server_dispatch(ctx, p->srv, p->sess, msg, &resp) == MCP_OK && resp != NULL);
    mcp_message_destroy(ctx, msg);
    char *out = mcp_message_serialize(ctx, resp);
    mcp_message_destroy(ctx, resp);
    assert(out != NULL);
    assert(fseek(p->s2c, 0, SEEK_END) == 0);
    assert(fputs(out, p->s2c) != EOF && fputc('\n', p->s2c) != EOF);
    assert(fflush(p->s2c) == 0);
...
static mcp_status_t pump_send(mcp_context_t *ctx, mcp_transport_t *t, const char *data,
    ...
    assert(fseek(p->c2s, 0, SEEK_END) == 0);
    assert(fwrite(data, 1, len, p->c2s) == len);
    assert(fputc('\n', p->c2s) != EOF);
    assert(fflush(p->c2s) == 0);
...
static mcp_status_t pump_recv(mcp_context_t *ctx, mcp_transport_t *t, char **line_out) {
    pump_t *p = (pump_t *)mcp_transport_backend(ctx, t);
    assert(fseek(p->s2c, p->rpos, SEEK_SET) == 0);
    static char buf[16384];
    assert(fgets(buf, (int)sizeof(buf), p->s2c) != NULL);
    p->rpos = ftell(p->s2c);
...
int main(void) {
    ...
    assert(mcp_server_add_tool(ctx, srv, mcp_tool_new(ctx, "echo", "Echo", schema, echo_handler,
                                                     NULL)) == MCP_OK);
    ...
    assert(mcp_client_call_tool(ctx, c, "echo", args, &cres) == MCP_OK && cres != NULL);
    ...
    assert(mcp_client_call_tool(ctx, c, "nope", NULL, NULL) == MCP_ERR_INVALID_ARGUMENT);
    assert(mcp_client_ping(ctx, c) == MCP_OK);
    assert(mcp_client_disconnect(ctx, c) == MCP_OK);
