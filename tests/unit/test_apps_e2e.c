...
static mcp_json_value_t *call_params(mcp_context_t *ctx, const char *name) {
    ...
    assert(p);
    assert(mcp_json_object_set(ctx, p, "name", mcp_json_string_new(ctx, name)) == MCP_OK);
    assert(mcp_json_object_set(ctx, p, "arguments", mcp_json_object_new(ctx)) == MCP_OK);
    return p;
}
...
int main(void) {
    ...
    mcp_tool_t *tool = mcp_tool_new(ctx, "get_system_status", "status", schema, status_handler,
                                    NULL);
    assert(tool);
    assert(mcp_tool_set_visibility(ctx, tool, MCP_TOOL_VIS_BOTH) == MCP_OK);
    assert(mcp_tool_require_perms(ctx, tool, MCP_APPS_PERM_CALL_TOOL) == MCP_OK);
    assert(mcp_server_add_tool(ctx, srv, tool) == MCP_OK);
    ...
    assert(plain);
    assert(mcp_session_revoke(ctx, plain, MCP_APPS_PERM_CALL_TOOL) == MCP_OK);
    ...
    assert(mcp_session_grant(ctx, plain, MCP_APPS_PERM_CALL_TOOL) == MCP_OK);
    r = dispatch_new(ctx, srv, plain, "p2", "tools/call",
                     call_params(ctx, "get_system_status"));
    const mcp_json_value_t *res = mcp_message_result(ctx, r);
    assert(res);
    assert(mcp_json_object_get(ctx, res, "_meta") == NULL);
    const mcp_json_value_t *st = mcp_json_object_get(ctx, res, "status");
    const char *sts = NULL;
    assert(mcp_json_string_value(ctx, st, &sts) == MCP_OK && strcmp(sts, "ok") == 0);
    mcp_message_destroy(ctx, r);
    ...
    assert(app);
    assert(mcp_session_set_apps_host(ctx, app, true) == MCP_OK);
    assert(mcp_session_grant(ctx, app, MCP_APPS_PERM_CALL_TOOL) == MCP_OK);
    mcp_apps_mount_t *h = NULL;
    assert(mcp_apps_mount(ctx, app, on_mount, on_unmount, NULL, &h) == MCP_OK);
    assert(mounted == 1);
    ...
    assert(rp && mcp_json_object_set(ctx, rp, "uri",
    ...
    assert(strstr(ts, "<meta http-equiv=\"Content-Security-Policy\"") == ts);
    assert(strstr(ts, "<div>ok</div>") != NULL);
    mcp_message_destroy(ctx, r);
