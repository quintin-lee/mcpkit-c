...
// s == NULL and n holding the value.
typedef struct {
    mcp_id_type_t type;
    char *s;
    double n;
...
// Growable array of mcp_id_entry_t. Allocation routed through the
// context allocator so counting allocators observe id-set growth.
// Destroy with the same ctx used at create time.
struct mcp_idset {
